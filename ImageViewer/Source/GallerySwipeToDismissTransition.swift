...
final class GallerySwipeToDismissTransition {
  ...
  func updateInteractiveTransition(
    horizontalOffset hOffset: CGFloat = 0, verticalOffset vOffset: CGFloat = 0
  ) {
  ...
  func finishInteractiveTransition(
    _ swipeDirection: SwipeToDismiss, touchPoint: CGFloat, targetOffset: CGFloat,
    escapeVelocity: CGFloat, completion: (() -> Void)?
  ) {
    ...
    UIView.animate(
      withDuration: expectedDuration * 0.65, delay: 0, usingSpringWithDamping: 1.0,
      initialSpringVelocity: springVelocity, options: UIView.AnimationOptions.curveLinear,
