...
func aspectFitSize(forContentOfSize contentSize: CGSize, inBounds bounds: CGSize) -> CGSize {
    ...
    return AVMakeRect(
        ...
        insideRect: CGRect(origin: CGPoint.zero, size: bounds)
    )
    .size
}
...
func aspectFitContentSize(forBoundingSize boundingSize: CGSize, contentSize: CGSize) -> CGSize {
    ...
    return AVMakeRect(
        aspectRatio: contentSize,
        insideRect: CGRect(origin: CGPoint.zero, size: boundingSize)
    )
    .size
}
...
func aspectFillZoomScale(forBoundingSize boundingSize: CGSize, contentSize: CGSize) -> CGFloat {
    ...
    let aspectFitSize = aspectFitContentSize(
        forBoundingSize: boundingSize,
        contentSize: contentSize
    )
    return (floor(boundingSize.width) == floor(aspectFitSize.width))
        ? (boundingSize.height / aspectFitSize.height) : (boundingSize.width / aspectFitSize.width)
}
...
func contentCenter(forBoundingSize boundingSize: CGSize, contentSize: CGSize) -> CGPoint {
    ...
    return CGPoint(
        x: contentSize.width * 0.5 + horizontalOffset,
        y: contentSize.height * 0.5 + verticalOffset
    )
}
...
func rotationAdjustedBounds() -> CGRect {
    ...
    if UIApplication.isPortraitOnly {
        ...
        return (UIDevice.current.orientation.isLandscape)
            ? CGRect(origin: CGPoint.zero, size: applicationWindow.bounds.size.inverted())
            : applicationWindow.bounds
    }
...
func maximumZoomScale(forBoundingSize boundingSize: CGSize, contentSize: CGSize) -> CGFloat {
    ...
    return min(boundingSize.width, boundingSize.height) / min(contentSize.width, contentSize.height)
        * 4
}
