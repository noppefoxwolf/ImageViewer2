...
open class GalleryViewController: UIPageViewController, ItemControllerDelegate {
  ...
  private var seeAllCloseLayout = ButtonLayout.pinRight(8, 16)
  private var thumbnailsLayout = ButtonLayout.pinLeft(8, 16)
  private var deleteLayout = ButtonLayout.pinRight(8, 66)
  ...
  public init(
    ...
    pagingDataSource = GalleryPagingDataSource(
      itemsDataSource: itemsDataSource, displacedViewsDataSource: displacedViewsDataSource,
    ...
    self.setViewControllers(
      [initialController], direction: UIPageViewController.NavigationDirection.forward,
  ...
  private func configureCloseButton() {
    ...
    if let closeButton {
      closeButton.addTarget(
        self, action: #selector(GalleryViewController.closeInteractively), for: .touchUpInside)
      closeButton.alpha = 0
  ...
  private func configureThumbnailsButton() {
    ...
    if let thumbnailsButton {
      thumbnailsButton.addTarget(
        self, action: #selector(GalleryViewController.showThumbnails), for: .touchUpInside)
  ...
  private func configureDeleteButton() {
    ...
    if let deleteButton {
      deleteButton.addTarget(
        self, action: #selector(GalleryViewController.deleteItem), for: .touchUpInside)
  ...
  private func layoutFooterView() {
    ...
    case .center(let marginBottom):
      ...
      footerView.frame.origin.y =
        self.view.bounds.height - footerView.bounds.height - marginBottom - defaultInsets.bottom
    ...
    case .pinBoth(let marginBottom, let marginLeft, let marginRight):
      ...
      footerView.frame.origin = CGPoint(
        x: marginLeft,
        y: self.view.bounds.height - footerView.bounds.height - marginBottom - defaultInsets.bottom)
  ...
  private func layoutScrubber() {
    ...
    scrubber.bounds = CGRect(
      origin: CGPoint.zero, size: CGSize(width: self.view.bounds.width, height: 40))
      ...
      setViewControllers(
        ...
        completion: { finished in
          DispatchQueue.main.async(execute: { [weak self] in
            self?.setViewControllers(
              [imageViewController], direction: direction, animated: false, completion: nil)
          })
        })
    } else {
  ...
  @objc private func rotate() {
    ...
    UIView.animate(
      withDuration: rotationDuration, delay: 0, options: UIView.AnimationOptions.curveLinear,
  ...
  private func closeDecorationViews(_ completion: (() -> Void)?) {
    ...
    UIView.animate(
      withDuration: decorationViewsFadeDuration,
  ...
  public func itemController(
    _ controller: ItemController, didSwipeToDismissWithDistanceToEdge distance: CGFloat
  ) {
