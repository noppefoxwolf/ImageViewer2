...
extension CAShapeLayer {
    ...
    static func replayShape(_ fillColor: UIColor, triangleEdgeLength: CGFloat) -> CAShapeLayer {
        ...
        triangle.frame = CGRect(
            origin: CGPoint.zero,
            size: CGSize(width: altitude, height: triangleEdgeLength)
        )
        ...
        -> CAShapeLayer
    {
        ...
        container.bounds.size = CGSize(
            width: 2 * elementSize.width + elementDistance,
    ...
    static func circle(_ fillColor: UIColor, diameter: CGFloat) -> CAShapeLayer {
        ...
        let frame = CGRect(
            origin: CGPoint.zero,
            size: CGSize(width: diameter * 2, height: diameter * 2)
        )
        circle.frame = frame
    ...
    static func circlePlayShape(_ fillColor: UIColor, diameter: CGFloat) -> CAShapeLayer {
        ...
        let trianglePath = UIBezierPath.equilateralTriangle(
            diameter / 2,
            shiftBy: CGPoint(x: diameter / 3, y: diameter / 4)
        )
