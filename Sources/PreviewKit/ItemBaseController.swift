...
where T: ItemView {
  ...
  private func configureScrollView() {
    ...
    scrollView.maximumZoomScale = max(
      maximumZoomScale,
      aspectFillZoomScale(forBoundingSize: self.view.bounds.size, contentSize: itemView.bounds.size)
    )

    scrollView.delegate = self

    scrollView.addObserver(
      self, forKeyPath: "contentOffset", options: NSKeyValueObservingOptions.new, context: nil)
  }
  ...
  override open func viewDidLayoutSubviews() {
    ...
    if let size = itemView.image?.size, size != CGSize.zero {
      ...
      let aspectFitItemSize = aspectFitSize(
        forContentOfSize: size, inBounds: self.scrollView.bounds.size)
  ...
  public func scrollViewDidZoom(_ scrollView: UIScrollView) {
    ...
    itemView.center = contentCenter(
      forBoundingSize: scrollView.bounds.size, contentSize: scrollView.contentSize)
  }
  ...
  @objc func scrollViewDidDoubleTap(_ recognizer: UITapGestureRecognizer) {
    ...
    let aspectFillScale = aspectFillZoomScale(
      forBoundingSize: scrollView.bounds.size, contentSize: itemView.bounds.size)

    if scrollView.zoomScale == 1.0 || scrollView.zoomScale > aspectFillScale {

      let zoomRectangle = zoomRect(
        ForScrollView: scrollView, scale: aspectFillScale, center: touchPoint)

      UIView.animate(
        withDuration: doubleTapToZoomDuration,
  ...
  @objc func scrollViewDidSwipeToDismiss(_ recognizer: UIPanGestureRecognizer) {
    ...
    case .ended:
      self.handleSwipeToDismissEnded(
        swipingToDismiss, finalVelocity: currentVelocity, finalTouchPoint: currentTouchPoint)
      ...
      let image = displacedView.image
    {
      ...
      if presentationStyle == .displacement {
        ...
        UIView.animate(
          ...
          usingSpringWithDamping: displacementSpringBounce, initialSpringVelocity: 1,
          options: .curveEaseIn,
          animations: { [weak self] in
            ...
            animatedImageView.bounds.size =
              self?.displacementTargetSize(forSize: image.size) ?? image.size
  ...
  func findVisibleDisplacedView() -> DisplaceableView? {
    ...
    let validAreaFrame = self.view.frame.insetBy(
  ...
  public func dismissItem(alongsideAnimation: () -> Void, completion: @escaping () -> Void) {
    ...
    case .displacement:
      ...
      if var displacedView = self.findVisibleDisplacedView() {
        ...
        UIView.animate(
          withDuration: reverseDisplacementDuration,
          animations: { [weak self] in
            ...
            self?.itemView.center = displacedView.convert(
              displacedView.boundsCenter, to: self!.view)
  ...
  public func gestureRecognizerShouldBegin(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
    ...
    if (self.index == 0 && velocity.direction == .right)
      || (self.index == self.itemCount - 1 && velocity.direction == .left)
    {
