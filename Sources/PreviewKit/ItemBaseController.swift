...
where T: ItemView {
    ...
    private func configureScrollView() {
        ...
        scrollView.maximumZoomScale = max(
            maximumZoomScale,
            aspectFillZoomScale(
                forBoundingSize: self.view.bounds.size,
    ...
    @objc func scrollViewDidSwipeToDismiss(_ recognizer: UIPanGestureRecognizer) {
        ...
        if swipingToDismiss == nil {
            swipingToDismiss =
                (abs(currentVelocity.x) > abs(currentVelocity.y)) ? .horizontal : .vertical
        }
    ...
    func handleSwipeToDismissInProgress(
        ...
        forTouchPoint touchPoint: CGPoint
    ) {
        ...
        case (.horizontal, 0) where self.itemCount != 1:
            ...
            swipeToDismissTransition?
                .updateInteractiveTransition(horizontalOffset: min(0, -touchPoint.x))
        ...
        case (.horizontal, self.itemCount - 1) where self.itemCount != 1:
            ...
            swipeToDismissTransition?
                .updateInteractiveTransition(horizontalOffset: max(0, -touchPoint.x))
    ...
    public func presentItem(alongsideAnimation: () -> Void, completion: @escaping () -> Void) {
            ...
            let image = displacedView.image
        {
            ...
            if presentationStyle == .displacement {
                ...
                UIView.animate(
                    withDuration: displacementDuration,
                    delay: 0,
    ...
    func findVisibleDisplacedView() -> DisplaceableView? {
        ...
        let validAreaFrame = self.view.frame.insetBy(
            dx: displacementInsetMargin,
            dy: displacementInsetMargin
        )
    ...
    public func gestureRecognizerShouldBegin(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
        ...
        guard velocity.orientation == .horizontal else {
            return swipeToDismissMode.contains(.vertical)
        }
